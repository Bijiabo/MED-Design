...
var ModeList = React.createClass({displayName: "ModeList",
    ...
    render : function(){
        ...
        var ModeListItemNodes = this.props.data.map(function(modeDataItem,index){
            ...
            return (
                React.createElement(ModeListItem, {state: {data:modeDataItem,index:index}, active: active, key: index})
            )
...
var ModeListItem = React.createClass({displayName: "ModeListItem",
    switchMode : function(){
        ...
        modeActiveIndex = this.state.data.index;
        this.setState({
            data:{
                id : "beforeSleep",
                name : "睡前"+ new Date()
