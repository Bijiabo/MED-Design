...
var PlayList = React.createClass({displayName: "PlayList",
    ...
    componentDidMount : function(){
        ...
        $.getJSON(this.props.dataUrl,{},function(playListData){
            self.setState({
                playList:playListData[activeMode]
            });
        });
        mountTip("PlayList");
    },
    render : function(){
        console.log(this.state.playList[activeMode]);
        for (var key in this.state.playList[activeMode])
        {
            console.log(key);
            console.log(this.state.playList[activeMode][key]);
            console.log(this.state.playList[activeMode].title);
        }
/*
        var PlayListItemNodes = this.state.playList[activeMode].playList.map(function(playListDataItem,index){
            return (
                <PlayListItem />
            );
        });
